...
describe('product model', () => {
  ...
  describe('test product model process', () => {
    ...
    it('Should Successfully Pass Test and return testProduct when called with ID', async () => {
      const returnedProduct = await productModel.getSpecificProduct(productTest.id as string)
    ...
    it('Should Successfully Pass Test and return product with edited attributes', async () => {
      const updatedProduct = await productModel.updateProduct(productTest.id as unknown as string, {
        ...productTest,
        price: 22
      })
    ...
    it('Should Successfully Pass Test and delete product from DB', async () => {
      const deletedUser = await productModel.deleteProduct(productTest.id as string)
