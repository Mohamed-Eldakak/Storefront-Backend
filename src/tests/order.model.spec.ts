...
describe('order model', () => {
  ...
  beforeAll(async () => {
    ...
    const createOrderToDeleteTest = await orderModel.createOrder(orderToDelete.user_id)
    orderToDelete.id = createOrderToDeleteTest.id

    const createOrderToproductsTest = await orderModel.createOrder(orderToAddProducts.user_id)
  ...
  describe('test order model process', () => {
    it('Should Successfully Pass Test return a new order', async () => {
      const createdorder = await orderModel.createOrder(user.id as unknown as string)
    ...
    it('Should Successfully Pass Test and return all available orders in DB', async () => {
      const orders = await orderModel.getAllOrders(user.id as unknown as string)
      expect(orders.length).toBe(4)
    })

    it('Should Successfully Pass Test and return active orders when called with ID', async () => {
      const activeOrder = await orderModel.getActiveOrders(user.id as unknown as string)
    ...
    it('Should Successfully Pass Test and return complete orders', async () => {
      const completeOrders = await orderModel.getCompleteOrders(user.id as unknown as string)
    ...
    it('Should Successfully Pass Test and return updated quantity of products added to order', async () => {
      const updateProductQuantityInOrder = await orderModel.updateProductQuantityInOrder(
        orderToAddProducts.id as unknown as string,
        product.id as unknown as string,
        15
      )
