...
class OrderModel {
  ...
  async createOrder(user_id: string): Promise<OrderType> {
    try {
      ...
      throw `Unable to create order according to ${error}`
    }
  }
  ...
  async getAllOrders(user_id: string): Promise<OrderType[]> {
    try {
      // connect with data base and create query
      const connection = await db.connect()
      const getActiveOrders = await connection.query('SELECT * FROM orders WHERE user_id = $1 ', [
        user_id
      ])
      connection.release()
      // return all orders
  ...
  async deleteProductFromOrder(order_id: string, product_id: string): Promise<ProductOrdersType> {
    try {
      // connect with data base and create query
      const connection = await db.connect()
