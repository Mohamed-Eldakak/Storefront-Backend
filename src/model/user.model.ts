...
class UserModel {
  ...
  async deleteUser(id: string): Promise<UserType> {
    try {
      // connect with data base and create query
      const connection = await db.connect()
      const deleteUser = await connection.query('DELETE FROM users WHERE id=($1) RETURNING *', [id])
      // release connection
      connection.release()
      // return deleted user
  ...
  async authenticate(email: string, password: string): Promise<UserType | null> {
    try {
      const connection = await db.connect()
      const getPassword = await connection.query('SELECT password FROM users WHERE email=($1)', [
        email
      ])
